...
> **Current release: v0.10.0** (Dogfood-hardening release: a **bounded transaction-gate wait** (`--tx-wait-timeout-ms`, default 5000ms) that turns concurrent `begin` hangs into a clear, metered error (`powdb_tx_gate_timeouts_total`) — fixing the ~54% concurrent-create failure mode from an internal dogfood run — plus opt-in **`--db-name`** connect enforcement, PowQL **reserved-word errors + backtick identifier quoting**, **`if (not) exists`** DDL idempotency, and **`schema`/`describe`** introspection. **Breaking:** `schema` and `describe` are now PowQL keywords (backtick-quote them to use as identifiers). Packaging: the TS client now ships a **dual ESM+CJS** build (fixes `require()` from CommonJS consumers) and the Docker image is **multi-arch** (`linux/amd64` + `linux/arm64`). Also fixes a pre-existing rollback bug that could poison a unique index on disk (affects ≤0.9.0) and hardens wire error-message sanitization + non-UTF-8 argv handling. On-disk formats unchanged — 0.9.x databases/backups open as-is. `@zvndev/powdb-sync` remains beta-gated and NOT published to npm.).
