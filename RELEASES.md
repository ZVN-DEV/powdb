...
> **Current release: v0.22.0.** The provable round, with behavior changes. Explicit transactions now have a default-on maximum lifetime (`POWDB_TX_MAX_LIFETIME_MS`, five minutes): a `begin` still open after that is rolled back and the connection closed, ending the one-connection-blocks-everyone hold on the write gate; raise it for migrations or set `0` to opt out. Int and Float now compare numerically in filters on every access path with no precision loss at any magnitude, so an indexed filter can no longer match rows the same filter misses without the index. Link cardinality is re-derived on every read from the target key's unique index rather than a stored byte, so DDL order no longer changes traversal behavior. Wire protocol version negotiation ships (hello blocks on `Connect`/`ConnectOk`, error class 10 on mismatch, conformance vectors), backward compatible in both directions with no forced upgrade order. Correctness fixes: a prepared `update` on a non-unique-int index silently lost the write; projection and aggregate fast paths read stale materialized views; a materialized view over a per-row-typed projection could abort the process or bit-reinterpret values (found by the new `fuzz_execute` target); integer arithmetic that overflows returns Empty instead of saturating. A differential oracle now polices PowDB against SQLite in CI with a content-anchored divergence ledger that cannot rot silently.
