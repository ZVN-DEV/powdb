...
powdb-cli --version 0.24.0`. This page is the actual promise.
...
| Surface | Across a patch (`0.24.0` to `0.24.1`) | Across a minor (`0.24` to `0.25`) |
|---|---|---|
...
```bash
cargo install powdb-cli --version 0.24.0 --locked
```

```json
{ "dependencies": { "@zvndev/powdb-client": "0.24.0" } }
```
