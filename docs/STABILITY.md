...
powdb-cli --version 0.22.0`. This page is the actual promise.
...
| Surface | Across a patch (`0.22.0` to `0.22.1`) | Across a minor (`0.22` to `0.23`) |
|---|---|---|
...
```bash
cargo install powdb-cli --version 0.22.0 --locked
```

```json
{ "dependencies": { "@zvndev/powdb-client": "0.22.0" } }
```
